...
function _logo_nuage(FileAttachment) {
  return FileAttachment("logo_nuage.svg").image({ width: 200 });
}

function _3(html, pred_warning) {
  return html`${pred_warning}`;
}

function _display(dashboard, html) {
...
The surfaces are generated using a generalized additive model, or GAM, implemented with the _mgcv_ package ([Wood, 2017](https://doi.org/10.1201%2F9781315370279)). The models were generated based on data from the NuAge cohort; a group of relatively healthy community dwelling Canadians aged 67+ who contributed data about their lifestyle, health and diet to an ongoing study ([Gaudreau et al., 2007](https://doi.org/10.1089/rej.2007.0596)).
...
function _7(md) {
  return md`
The application was designed by [Serge-Étienne Parent](https://observablehq.com/@essicolo), 2022.
  `;
}
    ...
    Plot.plot({
      ...
      color: {
        ...
        label:
          ...
          predz_name +
          " = " +
          predz_roundedvalue[0].toFixed(2),
        domain: target_range,
        reverse: true,
      },
      x: {
        label: predx_name,
        domain: [predx_range[0] + 0.4 * predx_range[0], predx_range[1]],
      },
      y: {
        label: predy_name,
        domain: [predy_range[0] + 0.05 * predy_range[0], predy_range[1]],
      },
      marks: [
        Plot.ruleX([0]),
        Plot.ruleY([0]),
        Plot.rect(
          data.filter((d) => d["z"] == predz_roundedvalue),
          Plot.bin({}, { x: "x", y: "y", fill: "fit", inset: 1.5 })
        ),
      ],
    }),
    invalidation
...
function _predz_value(
  ...
  const input = Inputs.range(predz_range, {
    step: 0.01,
    label: predz_name + " value",
  });
  display("left", input, invalidation);
  return Generators.input(input);
}

function _predz_name(
  Inputs,
  predictor_names,
  display,
  invalidation,
  ...
  const input = Inputs.select(predictor_names.map((d) => d["hname"]).sort(), {
    label: "Micronutrient 3 (z)",
    value: "Cluster 3, PC1",
  });
  display("left", input, invalidation);
  return Generators.input(input);
...
function _pred_warning(preddup) {
  if (preddup)
    var message =
      '<div style = "padding: 5px; background-color: #f6f6f6; border: 1px solid #000;">⚠️ Error: select three different predictors</div>';
  else var message = "";
  return message;
}

function _target_range(findrange, data) {
  return findrange(data, "fit");
}

function _predx_range(findrange, data) {
  return findrange(data, "x");
}

function _predy_range(findrange, data) {
  return findrange(data, "y");
}

function _predz_range(findrange, data) {
  return findrange(data, "z");
}
...
function _predictor_names() {
  return [
    ...
    {
      cname: "Poly._F._Acids",
      hname: "Polyunsaturated Fatty Acids",
      weight: 18,
    },
  ];
}

function _target_names() {
  return [
...
function _findrange() {
  return function findrange(data, column) {
...
function _filename(aq) {
  return function filename(
    ...
    // target string
    const targetname_s = target_names.find((d) => d["hname"] == target)[
      "cname"
    ];
...
function _preddup(predx_name, predy_name, predz_name) {
  ...
  const isDuplicate = preds.some(
    (item, index) => index !== preds.indexOf(item)
  );
  return isDuplicate;
}
...
function _jszip(require) {
  return require("jszip@3/dist/jszip.min.js");
}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    [
      ...
      {
        url: new URL(
          "./files/3cf92498184248d47c5f9b645a675d47a61d4630af717a3cbb6534d21fd2eca4b5aa130f01efd33d5d3853486c2d727f5a8b3cc162068fdd6689187a1d3cb15b.svg",
          import.meta.url
        ),
  ...
  main
    .variable(observer("logo_nuage"))
    .define("logo_nuage", ["FileAttachment"], _logo_nuage);
  main.variable(observer()).define(["html", "pred_warning"], _3);
  main
    .variable(observer("viewof display"))
    .define("viewof display", ["dashboard", "html"], _display);
  main
    .variable(observer("display"))
    .define("display", ["Generators", "viewof display"], (G, _) => G.input(_));
  ...
  main
    .variable(observer("predz_name"))
    .define(
      "predz_name",
      ["Inputs", "predictor_names", "display", "invalidation", "Generators"],
      _predz_name
    );
  main
    .variable(observer("predy_name"))
    .define(
      "predy_name",
      ["Inputs", "predictor_names", "display", "invalidation", "Generators"],
      _predy_name
  ...
  main
    .variable(observer("target_name"))
    .define(
      "target_name",
      ["Inputs", "target_names", "display", "invalidation", "Generators"],
      _target_name
    );
  main
    .variable(observer("model_name"))
    .define(
      "model_name",
      ["Inputs", "display", "invalidation", "Generators"],
      _model_name
    );
  main.variable(observer()).define(["md"], _17);
  main
    .variable(observer("selected_file"))
    .define(
      "selected_file",
      [
        "filename",
  ...
  main
    .variable(observer("pred_warning"))
    .define("pred_warning", ["preddup"], _pred_warning);
  main
    .variable(observer("target_range"))
    .define("target_range", ["findrange", "data"], _target_range);
  main
    .variable(observer("predx_range"))
    .define("predx_range", ["findrange", "data"], _predx_range);
  main
    .variable(observer("predy_range"))
    .define("predy_range", ["findrange", "data"], _predy_range);
  main
    .variable(observer("predz_range"))
    .define("predz_range", ["findrange", "data"], _predz_range);
  main
    .variable(observer("predz_roundedvalue"))
    .define("predz_roundedvalue", ["data", "predz_value"], _predz_roundedvalue);
  main
    .variable(observer("predictor_names"))
    .define("predictor_names", _predictor_names);
